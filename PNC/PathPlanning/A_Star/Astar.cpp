...
Astar::Astar(double resoluition, double robotRadius) : resolution(resoluition), robot_radius(robotRadius) {}

/**
 * 得到障碍物信息图 有障碍物的地方标记为 true 没有标记的为 false
 * @param ox 障碍物的 x 坐标集合
 * @param oy 障碍物的 y 坐标集合
*/
...
pair<vector<double>, vector<double>> Astar::planning(vector<double> start, vector<double> goal) {
    ...
    double gx = goal[0], gy = start[1];

    Node* start_node = new Node(calXyIndex(sx, min_x), calXyIndex(sy, min_y), 0.0, -1);
    Node* goal_node = new Node(calXyIndex(gx, min_y), calXyIndex(gy, min_y), 0.0, -1);
    ...
    while (true) {
        ...
        for (auto move : motion) {
            ...
            if (closed_set.find(n_id) != closed_set.end()) continue; // 已经在 closed_set 中

            if (!verifyNode(node)) continue; // 如果超出边界或者碰到障碍物

            if (open_set.find(n_id) == open_set.end()) { // 如果open_set 中没有这个节点
